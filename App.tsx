...
import { 
  ...
  ArrowUp, CornerDownLeft, Eye, EyeOff, Zap
} from 'lucide-react';
...
const Typewriter = ({ text = "", speed = 5, delay = 0, triggerKey = null }) => {
  const safeText = text || "";
...
const GlobalStyles = () => (
  <style>{`
    ...
    /* MONOCHROME FILTER */
    .monochrome-mode { filter: grayscale(100%) contrast(110%); }
  `}</style>
);
...
const SelectionCheckbox = ({ isSelected, onToggle, isTerminal }) => (
    ...
    >
        {isSelected 
            ? <div className={`p-0.5 rounded-none ${isTerminal ? 'bg-[#3fb950] text-black' : 'bg-black text-white'}`}><Check size={14} strokeWidth={4} /></div>
...
const NoteCard = ({ note, isTerminal, onOpen, onPeek, refreshKey, isSelectionMode, isSelected, onToggleSelect }) => {
  ...
  const lightClass = `bg-white border ${isSelected ? 'border-black ring-1 ring-black/20' : 'border-gray-300'} shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1),0_2px_4px_-1px_rgba(0,0,0,0.06)] hover:shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1),0_4px_6px_-2px_rgba(0,0,0,0.05)] hover:-translate-y-0.5`;
  const cardClass = isTerminal ? terminalClass : lightClass;
  
  const getPeekContent = (text) => {
    if (!text) return "Empty...";
    if (text.length <= 400) return text; // Increased Limit
  ...
  return (
    ...
    >
       <div className={`flex items-start p-3 border-b ${isTerminal ? 'bg-[#000000] border-[#30363d]' : 'bg-gray-50 border-gray-200'} min-h-[50px] flash-active cursor-pointer`}>
          {isSelectionMode && <SelectionCheckbox isSelected={isSelected} onToggle={onToggleSelect} isTerminal={isTerminal} />}
          <div className="flex-1 overflow-hidden">
             <div className="flex items-center gap-2 mb-1">
                <FileText size={10} className={isTerminal ? 'text-[#3fb950]' : 'text-gray-800'} />
                <span className={`font-bold text-[9px] font-mono uppercase tracking-wide opacity-50`}>DOC</span>
             </div>
             <span className={`font-bold text-xs font-mono leading-snug line-clamp-3 ${isTerminal ? 'text-[#e6edf3]' : 'text-gray-900'}`}>
          ...
          <div className="overflow-hidden">
             <div className="p-3 pt-1 cursor-pointer flex-1 hover:bg-current hover:bg-opacity-5 transition-colors flash-active" onClick={(e) => { e.stopPropagation(); onOpen(); }}>
                <div className={`text-[10px] leading-relaxed font-mono ${isTerminal ? 'text-[#8b949e]' : 'text-gray-600'}`}>
                    <div className={`pl-2 py-1 border-l-2 ${isTerminal ? 'border-[#3fb950] text-[#e6edf3]' : 'border-gray-800 text-gray-800'}`}>
       ...
       {!isSelectionMode && (
           <button 
             ...
             className={`w-full py-1.5 flex items-center justify-center border-t ${isTerminal ? 'border-[#30363d] text-white' : 'border-gray-200 text-gray-500 hover:text-black hover:bg-gray-100'} text-[9px] font-bold uppercase tracking-wider transition-all bg-transparent flash-active z-10`}
           >
...
const FolderCard = ({ folder, isTerminal, onToggle, onMoveNote, onDeleteNote, onOpenNote, onPeekNote, onAddNote, refreshKey, isSelectionMode, isSelected, onToggleSelect, selectedSubItems = new Set(), onToggleSubItem }) => {
  ...
  return (
    <div className={`relative flex flex-col w-full rounded-none transition-all duration-200 ease-spring overflow-hidden ${cardClass}`}>
      <div 
        className={`p-3 flex items-start cursor-pointer flash-active transition-colors duration-200 ${folder.isExpanded ? (isTerminal ? 'border-b border-[#30363d]' : 'border-b border-gray-200') : ''} ${isTerminal ? '' : 'bg-gray-50'}`}
        onClick={() => isSelectionMode ? onToggleSelect() : onToggle()}
      >
        {isSelectionMode && <SelectionCheckbox isSelected={isSelected} onToggle={onToggleSelect} isTerminal={isTerminal} />}

        <div className="flex-1 flex items-start gap-2 overflow-hidden">
           <Folder size={14} className={`mt-0.5 transition-colors duration-200 ${isTerminal ? 'text-[#3fb950]' : 'text-gray-800'}`} />
        ...
        <div className="overflow-hidden">
           ...
           <div className="p-2 bg-transparent grid grid-cols-2 gap-2">
              ...
              folder.notes.map(note => (
                 <div key={note.id} className={`flex flex-col col-span-1 ${isTerminal ? 'bg-[#0d1117] border border-[#30363d]' : 'bg-white border border-gray-200'} rounded-none overflow-hidden transition-transform active:scale-[0.98] ${isSelectionMode && selectedSubItems.has(note.id) ? (isTerminal ? 'border-[#3fb950]' : 'border-black') : ''}`}>
                   <div 
                      ...
                      {isSelectionMode && (
                          <div className="mr-2">
                              {selectedSubItems.has(note.id) 
                                  ? <Check size={12} className={isTerminal ? 'text-[#3fb950]' : 'text-black'} /> 
                   ...
                   <div className={`grid transition-all duration-200 ease-spring ${!isSelectionMode && note.isPeeked ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                     <div className="overflow-hidden">
                       <div 
                         className={`mx-2 mb-2 mt-1 pl-2 py-1 border-l-2 text-[9px] font-mono whitespace-pre-wrap cursor-pointer ${isTerminal ? 'border-[#3fb950] text-[#e6edf3]' : 'border-black text-gray-700'}`}
              ...
              {!isSelectionMode && (
                  <button 
                      onClick={onAddNote}
                      className="col-span-2 w-full py-1.5 text-[10px] text-center opacity-40 hover:opacity-100 border border-dashed border-current rounded-none flex justify-center items-center gap-1 hover:bg-current hover:bg-opacity-5 transition-all font-mono flash-active"
                  >
...
const EditorModal = ({ note, isTerminal, onClose, onSave }) => {
  ...
  const lineCount = Math.max(stats.lines, 1); 
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col ${isTerminal ? 'bg-[#000000] text-[#e6edf3]' : 'bg-white text-[#24292f]'} font-mono animate-in slide-in-from-bottom-2 fade-in duration-150`}>
      <div className={`flex justify-between items-center p-2 px-4 border-b ${isTerminal ? 'border-[#30363d]' : 'border-[#d0d7de]'}`}>
        <div className="flex flex-col gap-1 overflow-hidden flex-1 mr-4">
            ...
            <div className={`flex gap-3 text-[9px] opacity-60`}>
                ...
                <span className={`px-1 rounded-none text-[8px] font-bold ${isTerminal ? 'bg-[#238636] text-white' : 'bg-black text-white'}`}>INSERT</span>
            </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden relative" ref={containerRef}>
          ...
          <div 
            ref={lineNumbersRef}
            className={`pt-4 pb-4 pr-3 pl-2 text-[11px] leading-[20px] opacity-30 select-none border-r text-right overflow-hidden ${isTerminal ? 'border-[#30363d] bg-[#0d1117]' : 'border-gray-200 bg-gray-50'} w-10`}
          >
              {lines.map(line => (
          ...
          <textarea 
            ...
            className={`flex-1 w-full p-4 pt-4 pb-4 bg-transparent outline-none resize-none text-[11px] leading-[20px] font-mono whitespace-pre-wrap`}
...
export default function DesnoteAppV7() {
  ...
  const [isMonochrome, setIsMonochrome] = useState(false); // NEW STATE
  ...
  const bgClass = isTerminal ? 'bg-[#000000]' : 'bg-[#f6f8fa]';
  const textClass = isTerminal ? 'text-[#e6edf3]' : 'text-[#1f2328]';
  const modalBg = isTerminal ? 'bg-[#0d1117] border border-[#30363d] text-[#e6edf3]' : 'bg-white border border-gray-200 text-gray-800 shadow-2xl rounded-none';
  ...
  return (
    <div className={`min-h-screen w-full relative flex flex-col ${bgClass} ${textClass} font-mono transition-colors duration-200 overflow-hidden ${isMonochrome ? 'monochrome-mode' : ''}`}>
      ...
      <div className={`w-full flex items-center justify-between py-1 px-4 text-[9px] font-mono border-b select-none ${isTerminal ? 'bg-[#000000] border-[#30363d] text-[#8b949e]' : 'bg-white border-gray-200 text-gray-400'}`}>
      ...
      <header className={`px-4 py-3 flex flex-col gap-3 z-10 sticky top-0 ${isTerminal ? 'bg-[#000000]/95 border-b border-[#30363d]' : 'bg-white/80 border-b border-gray-200 backdrop-blur-md shadow-sm'}`}>
        <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">{isTerminal ? <Terminal className="text-[#e6edf3]" size={18} /> : <Github className="text-gray-900" size={18} />}<h1 className="text-lg font-bold tracking-tight">DESNOTE <span className="text-[10px] font-normal opacity-50 ml-1 border px-1 rounded-none">v9.2</span></h1></div>
            <div className="flex items-center gap-3">
              <button onClick={toggleSelectionMode} className={`px-4 py-1.5 rounded-none text-[10px] font-bold border transition-all ${isSelectionMode ? (isTerminal ? 'bg-[#238636] border-[#3fb950] text-white' : 'bg-black border-black text-white shadow-lg') : (isTerminal ? 'border-[#30363d] hover:border-[#8b949e]' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-400 shadow-sm')}`}>{isSelectionMode ? 'DONE' : 'SELECT'}</button>
        ...
        {!searchQuery && (
          <div className="flex gap-2 w-full overflow-x-auto pb-1 scrollbar-none items-center">
             <button onClick={() => toggleViewMode('FOLDER')} className={`flex items-center justify-center gap-2 px-3 py-1.5 rounded-none border text-[10px] transition-all select-none flash-active ${viewMode === 'FOLDER' ? (isTerminal ? 'bg-[#1f242e] border-[#3fb950] text-[#e6edf3]' : 'bg-gray-100 border-gray-300 text-gray-800 font-bold') : (isTerminal ? 'bg-transparent border-[#30363d] text-[#8b949e]' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50')}`}><Folder size={12}/> <span>FOLDERS</span> <span className="opacity-50">{folders.length}</span></button>
             <button onClick={() => toggleViewMode('NOTE')} className={`flex items-center justify-center gap-2 px-3 py-1.5 rounded-none border text-[10px] transition-all select-none flash-active ${viewMode === 'NOTE' ? (isTerminal ? 'bg-[#1f242e] border-[#3fb950] text-[#e6edf3]' : 'bg-gray-100 border-gray-300 text-gray-800 font-bold') : (isTerminal ? 'bg-transparent border-[#30363d] text-[#8b949e]' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50')}`}><FileText size={12}/> <span>NOTES</span> <span className="opacity-50">{rootNotes.length}</span></button>
             <button onClick={() => toggleViewMode('ALL')} className={`flex items-center justify-center gap-2 px-3 py-1.5 rounded-none border text-[10px] transition-all select-none flash-active ${viewMode === 'ALL' ? (isTerminal ? 'bg-[#1f242e] border-[#3fb950] text-[#e6edf3]' : 'bg-gray-100 border-gray-300 text-gray-800 font-bold') : (isTerminal ? 'bg-transparent border-[#30363d] text-[#8b949e]' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50')}`}><Disc size={12}/> <span>ALL</span></button>
          </div>
        )}
        <div className={`relative w-full group`}><div className={`absolute left-3 top-1/2 -translate-y-1/2 opacity-50 font-mono text-xs flex items-center gap-1`}><Command size={12}/> {">"}</div><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="grep..." className={`w-full py-2.5 pl-12 pr-4 text-xs bg-transparent border rounded-none outline-none transition-all font-mono ${isTerminal ? 'border-[#30363d] focus:border-[#58a6ff] placeholder-[#8b949e] bg-[#0d1117]' : 'border-gray-200 bg-white focus:ring-2 ring-gray-100 placeholder-gray-400 shadow-sm'}`}/></div>
      </header>
              ...
              >
                  ...
                  <div className={`flex flex-col w-full mb-1 absolute top-2 left-0 right-0 gap-2 items-center transition-all duration-200 ${addMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                     <button 
                        ...
                        className={`p-2 rounded-none hover:bg-current hover:bg-opacity-10 transition-colors ${isTerminal ? 'text-[#3fb950]' : 'text-gray-900'}`}
                        title="New Note"
                     >
                        <FileText size={20} />
                     </button>
                     <button 
                        ...
                        className={`p-2 rounded-none hover:bg-current hover:bg-opacity-10 transition-colors ${isTerminal ? 'text-[#1f6feb]' : 'text-gray-900'}`}
                        title="New Folder"
                     >
                        <Folder size={20} />
                     </button>
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in duration-150">
            <div className={`w-full max-w-sm p-6 ${modalBg} flex flex-col gap-6 shadow-2xl font-mono`}>
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <button onClick={toggleMonochrome} className="w-full py-3 px-4 rounded-none border border-current border-opacity-20 flex items-center justify-between hover:bg-current hover:bg-opacity-5 transition-all flash-active">
                        <div className="flex items-center gap-3">
                            {isMonochrome ? <EyeOff size={16} /> : <Eye size={16} />}
                            <span className="font-bold text-xs">Monochrome Mode</span>
                        </div>
                        <span className={`text-[10px] opacity-60 uppercase px-2 py-0.5 rounded-none ${isMonochrome ? 'bg-white text-black font-bold' : 'bg-current bg-opacity-10'}`}>{isMonochrome ? 'ON' : 'OFF'}</span>
                    </button>
                    ...
                    <button onClick={handleExport} className="w-full py-3 px-4 rounded-none border border-current border-opacity-20 flex items-center gap-4 hover:bg-current hover:bg-opacity-5 transition-all flash-active">
                        <div className={`p-1.5 rounded-none ${isTerminal ? 'bg-[#3fb950]/20 text-[#3fb950]' : 'bg-gray-200 text-gray-800'}`}>
                            <Download size={16}/>
                        </div>
                    ...
                    <button onClick={() => fileInputRef.current.click()} className="w-full py-3 px-4 rounded-none border border-current border-opacity-20 flex items-center gap-4 hover:bg-current hover:bg-opacity-5 transition-all flash-active">
                        <div className={`p-1.5 rounded-none ${isTerminal ? 'bg-yellow-500/20 text-yellow-500' : 'bg-gray-200 text-gray-800'}`}>
                            <Upload size={16}/>
                        </div>
                        <div className="text-left flex-1">
